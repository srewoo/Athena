#!/usr/bin/env python3
"""
Load default domain context into ChromaDB

This script stores the sales enablement domain context as the default
context in ChromaDB, making it immediately available for RAG retrieval.
"""
...
# Default domain context for Sales Enablement domain
...
async def load_default_domain_context():
    """Load default domain context into ChromaDB"""
    print("=" * 60)
    print("Loading Default Domain Context into ChromaDB")
    print("=" * 60)
    print()

    # Initialize service
    print("Initializing domain context service...")
    domain_service = get_domain_context_service()
    print("✅ Service initialized")
    print()

    # Store domain context
    print("Storing domain context for session: default")
    print(f"  Products: {len(DEFAULT_DOMAIN_CONTEXT['products'])} items")
    print(f"  Prospects: {len(DEFAULT_DOMAIN_CONTEXT['prospects'])} items")
    print(f"  Quality Principles: {len(DEFAULT_DOMAIN_CONTEXT['quality_principles'])} items")
    print(f"  Anti-Patterns: {len(DEFAULT_DOMAIN_CONTEXT['anti_patterns'])} items")
    print(f"  Failure Modes: {len(DEFAULT_DOMAIN_CONTEXT['failure_modes'])} items")
    print(f"  Few-Shot Examples: {len(DEFAULT_DOMAIN_CONTEXT['few_shot_examples'])} items")
    print()

    try:
        chunk_counts = await domain_service.store_domain_context(
            domain_context=DEFAULT_DOMAIN_CONTEXT,
            session_id="default",
            project_id="default"
        )
        ...
        # Verify storage
        stats = domain_service.get_stats(session_id="default")
        print(f"Total chunks stored: {stats['total_chunks']}")
        print(f"Categories: {list(stats['categories'].keys())}")
        print()

        print("✅ Default domain context is now available for RAG retrieval!")
        print()
        print("Next steps:")
        print("  1. Generate evals - they will use this validated context")
        print("  2. Context will be retrieved selectively based on dimension")
        print("  3. Only relevant chunks will be injected into prompts")

    except Exception as e:
        print(f"❌ Error storing domain context: {e}")
        import traceback
        traceback.print_exc()


if __name__ == "__main__":
    asyncio.run(load_default_domain_context())
